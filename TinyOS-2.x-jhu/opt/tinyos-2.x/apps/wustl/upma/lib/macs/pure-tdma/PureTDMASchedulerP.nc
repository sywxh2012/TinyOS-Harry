...
//Transmission schedule for plant #1,downlink by Lanshun
...
//Transmission schedule for plant #2,downlink by Lanshun
...
//Transmission schedule for downlink  of both plant
...
uint8_t schedule_len = 164;
uint32_t superframe_length = 49; //Real superframe length
				...
				for (i=0; i<schedule_len; i++){
					if (schedule[i][6]==flow_id_at_send_done){ //check flow id
						if(schedule[i][7] == root_id_at_send_done){//check root
							//if (TOS_NODE_ID == 181 || TOS_NODE_ID == 182 || TOS_NODE_ID == 154 || TOS_NODE_ID == 254){
  				...
  				if(TOS_NODE_ID == schedule[i][1] || TOS_NODE_ID == schedule[i][2]){//check sender & receiver id
  					if(schedule[i][10]>1){ //check if this is on a multi-hop path
  						...
  						if(TOS_NODE_ID == schedule[i][1] && schedule[i][8]==0){
	  						...
	  						//if emergency sensor, check if the emergency exists; judged by isIdle.
	  						if (TOS_NODE_ID == 181 || TOS_NODE_ID == 153 || TOS_NODE_ID == 152 || TOS_NODE_ID == 155 || TOS_NODE_ID == 182 || TOS_NODE_ID == 253 || TOS_NODE_ID == 252 || TOS_NODE_ID == 255){
	  							idleStatus = call SimMote.isIdle();
	  							if (idleStatus){
	  								if (TOS_NODE_ID == 181){
	  									//printf("***SENSOR:%u WILL NOT send because idleStatus is: %u\n", TOS_NODE_ID, idleStatus);
	  									//printf("sendDone Status for SENSOR:%u is: %u\n", TOS_NODE_ID, schedule[i][8]);
	  								}
	  							}else{
	  								//printf("***SENSOR:%u WILL send because idleStatus is: %u\n", TOS_NODE_ID, idleStatus);
	  								//printf("sendDone Status for SENSOR:%u is: %u\n", TOS_NODE_ID, schedule[i][8]);
	  								call SubSend.send(&packet, sizeof(TestNetworkMsg));
	  							}
	  						}else{
